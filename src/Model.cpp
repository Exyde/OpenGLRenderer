...
void Model::loadModel(std::string const &path){
    ...
    const aiScene *scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs); // Could use more flags here to optimize such as GenNormals, SplitLargeMeshes or OptimizeMeshes
...
void Model::processNode(aiNode* node, const aiScene* scene) {
    ...
    // if node contains any meshes
    for (unsigned int i =0; i < node->mNumMeshes; i++){
        aiMesh *mesh = scene->mMeshes[node->mMeshes[i]];
        meshes.push_back(processMesh(mesh, scene));
    }

    // then recursively for each children
