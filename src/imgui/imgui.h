...
#pragma clang diagnostic ignored "-Wnontrivial-memaccess"  // warning: first argument in call to 'memset' is a pointer
                                                           // to non-trivially copyable type
...
IMGUI_API float GetFontSize();  // get current scaled font size (= height in pixels). AFTER global scale factors
...
// - You can use most Docking facilities without calling any API. You don't necessarily need to call a DockSpaceXXX
// function to use Docking!
//   - Drag from window title bar or their tab to dock/undock. Hold SHIFT to disable docking.
//   - Drag from window menu button (upper-left button) to undock an entire node (all windows).
//   - When io.ConfigDockingWithShift == true, you instead need to hold SHIFT to enable docking.
// - Dockspaces:
//   - If you want to dock windows into the edge of your screen, most application can simply call
//   DockSpaceOverViewport():
//     e.g. ImGui::NewFrame(); then ImGui::DockSpaceOverViewport();  // Create a dockspace in main viewport.
//      or: ImGui::NewFrame(); then ImGui::DockSpaceOverViewport(0, nullptr, ImGuiDockNodeFlags_PassthruCentralNode); //
...
//   - DockSpaceOverViewport() basically creates an invisible window covering a viewport, and submit a DockSpace() into
//   it.
...
IMGUI_API bool IsMouseReleasedWithDelay(
    ...
    float delay);  // delayed mouse release (use very sparingly!). Generally used with 'delay >=
                   // io.MouseDoubleClickTime' + combined with a 'io.MouseClickedLastCount==1' test. This is a very
                   // rarely used UI idiom, but some apps use this: e.g. MS Explorer single click on an icon to rename.
...
enum ImGuiTreeNodeFlags_ {
    ...
    // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 16,  // FIXME:: Disable automatic scroll on TreePop() if node got
...
enum ImGuiHoveredFlags_ {
    ...
    // blocking access to this item/window. FIXME-TOD: Unavailable yet.
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ImGuiKey_COUNT = ImGuiKey_NamedKey_END,  // Obsoleted in 1.91.5 because it was extremely misleading (since named
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_AlphaNoBg = 1 << 12,  //              // ColorEdit, ColorPicker, ColorButton: disable rendering
...
struct ImGuiIO {
    ...
    bool ConfigNavMoveSetMousePos;     // = false          // Directional/tabbing navigation teleports the mouse cursor.
                                       // May be useful on TV/console systems where moving a virtual mouse is difficult.
    ...
    bool ConfigNavEscapeClearFocusWindow;  // = false          // Pressing Escape can clear focused window as well
                                           // (super set of io.ConfigNavEscapeClearFocusItem).
    ...
    bool ConfigViewportsNoDefaultParent;  // = true           // When false: set secondary viewports' ParentViewportId
                                          // to main viewport ID by default. Expects the platform backend to setup a
                                          // parent/child relationship between the OS windows based on this value. Some
                                          // backend may ignore this. Set to true if you want viewports to automatically
                                          // be parent of main viewport, otherwise all viewports will be top-level OS
                                          // windows.
    bool ConfigViewportsPlatformFocusSetsImGuiFocus;  //= true // When a platform window is focused (e.g. using Alt+Tab,
                                                      // clicking Platform Title Bar), apply corresponding focus on
                                                      // imgui windows (may clear focus/active id from imgui windows
                                                      // location in other platform windows). In principle this is
                                                      // better enabled but we provide an opt-out, because some Linux
                                                      // window managers tend to eagerly focus windows (e.g. on mouse
    ...
    bool
        ConfigMacOSXBehaviors;  // = defined(__APPLE__) // Swap Cmd<>Ctrl keys + OS X style text editing cursor movement
    ...
    bool ConfigScrollbarScrollByPage;  // = true           // Enable scrolling page by page when clicking outside the
                                       // scrollbar grab. When disabled, always scroll to clicked location. When
    ...
    bool ConfigDebugHighlightIdConflicts;  // = true           // Highlight and show an error message popup when
                                           // multiple items have conflicting identifiers.
...
struct ImGuiListClipper {
    ...
    // float items_height = -1.0f) { memset(this, 0, sizeof(*this)); ItemsCount = -1; Begin(items_count, items_height);
    // } // [removed in 1.79]
#endif
};
...
enum ImGuiMultiSelectFlags_ {
    ...
    ImGuiMultiSelectFlags_NoRangeSelect =
        1 << 2,  // Disable Shift+selection mouse/keyboard support (useful for unordered 2D selection). With BoxSelect
                 // is also ensure contiguous SetRange requests are not combined into one. This allows not handling
    ...
    ImGuiMultiSelectFlags_NoAutoClear =
        1
        << 4,  // Disable clearing selection when navigating or selecting another one (generally used with
    ...
    ImGuiMultiSelectFlags_BoxSelect1d =
        1 << 6,  // Enable box-selection with same width and same x pos items (e.g. full row Selectable()).
                 // Box-selection works better with little bit of spacing between items hit-box in order to be able to
                 // aim at empty space.
    ...
    ImGuiMultiSelectFlags_NavWrapX =
        1 << 16,  // [Temporary] Enable navigation wrapping on X axis. Provided as a convenience because we don't have a
                  // design for the general Nav API for this yet. When the more general feature be public we may
                  // obsolete this flag in favor of new one.
...
struct ImGuiMultiSelectIO {
    ...
    int ItemsCount;  //  ms:w, app:r     /        app:r   // 'int items_count' parameter to BeginMultiSelect() is copied
                     //  here for convenience, allowing simpler calls to your ApplyRequests handler. Not used
                     //  internally.
};
...
struct ImGuiSelectionBasicStorage {
    ...
    bool PreserveOrder;  // = false  // GetNextSelectedItem() will return ordered selection (currently implemented by
                         // two additional sorts of selection. Could be improved)
...
struct ImDrawCmd {
    ...
    void* UserCallbackData;    // 4-8  // Callback user data (when UserCallback != NULL). If called AddCallback() with
                               // size == 0, this is a copy of the AddCallback() argument. If called AddCallback() with
                               // size > 0, this is pointing to a buffer where data is stored.
...
struct ImFontConfig {
    ...
    float GlyphMinAdvanceX;  // 0        // Minimum AdvanceX for glyphs, set Min to align font icons, set both Min/Max
                             // to enforce mono-space font. Absolute value for default size, other sizes will scale this
                             // value.
    ...
    float RasterizerDensity;   // 1.0f     // [LEGACY: this only makes sense when ImGuiBackendFlags_RendererHasTextures
                               // is not supported] DPI scale multiplier for rasterization. Not altering other font
                               // metrics: makes it easy to swap between e.g. a 100% and a 400% fonts for a zooming
    // display, or handle Retina screen. IMPORTANT: If you change this it is expected that you
...
struct ImFontAtlas {
    ...
    IMGUI_API void ClearFonts();  // [OBSOLETE] Clear input+output font data (same as ClearInputData() + glyphs storage,
                                  // UV coordinates).
    IMGUI_API void ClearTexData();  // [OBSOLETE] Clear CPU-side copy of the texture data. Saves RAM once the texture
    ...
    ImTextureFormat TexDesiredFormat;  // Desired texture format (default to ImTextureFormat_RGBA32 but may be changed
                                       // to ImTextureFormat_Alpha8).
...
struct ImFontBaked {
    ...
    unsigned int LoadNoRenderOnLayout
        : 1;  // 0  //     // Enable a two-steps mode where CalcTextSize() calls will load AdvanceX *without*
              // rendering/packing glyphs. Only advantagous if you know that the glyph is unlikely to actually be
              // rendered, otherwise it is slower because we'd do one query on the first CalcTextSize and one query on
              // the first Draw.
...
struct ImFont {
    ...
    float LegacySize;  // 4     // in  // Font size passed to AddFont(). Use for old code calling PushFont() expecting
                       // to use that size. (use ImGui::GetFontBaked() to get font baked at current bound size).
...
enum ImGuiViewportFlags_ {
    ...
    ImGuiViewportFlags_CanHostOtherWindows =
        1 << 11,  // Viewport can host multiple imgui windows (secondary viewports are associated to a single window).
                  // // FIXME: In practice there's still probably code making the assumption that this is always and
                  // only on the MainViewport. Will fix once we add support for "no main viewport".
